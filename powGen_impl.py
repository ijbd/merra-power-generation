...
def get_lat_lon_new(processed_merra_file):
...
def main(year,log_file,processed_merra_file,destination_file_path):
    ...
    if path.exists(test_file_name) == False:
    ...
    # Issue: python script crashes after ~3 hours, so instead use a shell script and logfile to run one coordinate at a time
    l = open(log_file,"a+")
    l.write("%d\n" %longitude)
